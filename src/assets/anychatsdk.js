...
var ANYCHAT_CORESDKEVENT_DEVICEFAIL = 33; //设备打开失败
...
//录像图片
var BRAC_SO_RECORD_INSERTIMAGE = 146;
...
var bSupportHtml5 = false; // 是否支持HTML5
...
function BRAC_InitSDK(apilevel) {
    ...
    // //browser == "Safari" ||
    // if (isSurportH5 && !(browser == "IE" || browser == "Firefox")) {
    //     anychat = AnyChatSDK();
    ...
    //     bSupportMultiStream = true; //说不定待会儿就删掉
...
function BRAC_SetVideoPosEx(userid, parentobj, id, streamindex) {
    if (bSupportHtml5) {
        anychat.SetVideoPos(userid, parentobj, id);
        return 0;
    }
...
function BRAC_SetUserStreamInfo(dwUserId, dwStreamIndex, infoname, value) {
    ...
    if (typeof(value) == "string") // 原本是true
...
// ppt 控制
function BRAC_SDKControl(dwCtrlCode, strInJson) {
    return anychat.SDKControl(dwCtrlCode, strInJson);
}
